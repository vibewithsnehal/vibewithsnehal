"""Annotated replay rendering: court model, ball trail, calls, live tallies."""
...
def render_overlay_video(
    ...
    calls_by_frame = sorted(result.calls, key=lambda c: c.frame)
    ...
    for idx, frame in enumerate(frames):
        ...
        canvas = frame.copy()
        _draw_court_model(canvas, result)
        _draw_trail(canvas, positions, idx)
        _draw_calls(canvas, calls_by_frame, idx, persist)
...
def _draw_court_model(canvas: np.ndarray, result: AnalysisResult) -> None:
    for (p0, p1) in court.COURT_LINES.values():
        a = result.calibration.court_to_image(np.array([p0]))[0]
        b = result.calibration.court_to_image(np.array([p1]))[0]
        cv2.line(
            canvas,
            (int(a[0]), int(a[1])),
            (int(b[0]), int(b[1])),
...
def _draw_trail(canvas: np.ndarray, positions: dict, idx: int) -> None:
...
def _draw_calls(canvas: np.ndarray, calls: list, idx: int, persist: int) -> None:
    for call in calls:
        if not (call.frame <= idx <= call.frame + persist):
            continue
        ...
        _text(canvas, label, (x + 14, y - 10), color)
...
def _draw_hud(canvas: np.ndarray, result: AnalysisResult, idx: int) -> None:
    ...
    _text(canvas, "COURTVISION", (12, 24), (255, 255, 255))
    _text(canvas, f"rally {max(rally, 1)}  in {n_in}  out {n_out}", (12, 46), (200, 220, 255))


def _text(canvas: np.ndarray, s: str, org: tuple[int, int], color) -> None:
